...
describe('Dashboard', () => {
    ...
    it('should click the update bell in the second container and launch update', () => {
        const updateBell = element(by.css('app-info-container .update-bell'));

        updateBell.click().then(() => {
            const bellOnElem = updateBell.element('.green');
            expect(bellOnElem).not.toBeNull();
        });
    });
});
